...
export const SCHEDULE: DayData[] = [
  ...
  },
  {
    ...
    timeline: [
      { time: '05:30', emoji: '🛌', label: '기상 + 가벼운 스트레칭', status: 'pending', detail: '일출 05:42 · 정상 0-2°C 대비 양파 옷' },
      { time: '05:45', emoji: '🍙', label: '아파트 김밥 가볍게 + 따뜻한 차', status: 'pending', detail: '나머지 김밥은 점심·간식용 배낭에' },
      { time: '06:30', emoji: '🚶', label: '호텔 → Zermatt 역 → Gornergrat Bahn 역 도보 (5-10분)', status: 'pending' },
      { time: '07:00', emoji: '🚂', label: 'Gornergrat Bahn 탑승 (4명 2등석 왕복, STP 50% 할인 CHF 66/인)', status: 'pending', detail: '⭐ 진행방향 오른쪽 창가 = 마테호른 명당. CHF 264 (₩440K) · 매표소 또는 앱 사전 구매 권장' },
      { time: '07:34', emoji: '🏔️', label: 'Gornergrat 정상 도착 (3,089m) — 알펜글로우 마테호른', status: 'pending', detail: '🥶 0-2°C · 자켓·장갑·비니 필수' },
      { time: '07:35-08:30', emoji: '📸', label: '정상 산책 + 마테호른·29개 4,000m 봉우리·Gorner 빙하 사진', status: 'pending', detail: 'Monte Rosa Dufourspitze 4,634m (스위스 최고봉) + Liskamm + Breithorn + Dom 등' },
      { time: '08:30-09:30', emoji: '☕', label: 'Kulmhotel 3100 정상 카페 — 따뜻한 음료 + 핫초코', status: 'pending', detail: '실내 휴식 + 안개 시 안개 걷히길 기다리는 baseline' },
      { time: '09:30', emoji: '🚂', label: 'Gornergrat → Rotenboden 한 정거장 하행 (같은 Gornergrat Bahn)', status: 'pending' },
      { time: '09:45-11:00', emoji: '🥾', label: 'Riffelsee 트레일 (Rotenboden ↔ 호수, 왕복 1h, 평탄)', status: 'pending', detail: '⭐ 마테호른 반영 인스타 명소 #1 · 호수 한 바퀴 + 사진 여유 · 김밥 남았으면 호수 옆에서 🍙🏔️' },
      { time: '11:15', emoji: '🚂', label: 'Rotenboden → Zermatt 하행 (왕복 티켓에 포함)', status: 'pending', detail: '~30분 · 12시 전 안전하게 시내 복귀' },
      { time: '12:00', emoji: '🍽️', label: '체르마트 시내 가벼운 점심 (저녁 든든하니 가볍게)', status: 'pending', detail: 'Bahnhofstrasse 노천 카페 또는 간단 비스트로' },
      { time: '13:00-14:00', emoji: '☕', label: '카페 또는 호텔 휴식 + MeteoSwiss 천둥 ETA 확인', status: 'pending', detail: '⚠️ 천둥 예보 있으면 5-호수길 취소 → 마을 산책 모드로 전환' },
      { time: '14:00', emoji: '🚠', label: 'Sunnegga 푸니쿨라 (Matterhorn-Express 역, STP 50% 할인)', status: 'pending', detail: '~3분 ascent · 1,608m → 2,288m' },
      { time: '14:10', emoji: '🚠', label: 'Sunnegga → Blauherd 케이블카 환승 (5-호수길 들머리)', status: 'pending', detail: '~5분 · 2,288m → 2,571m' },
      { time: '14:20-16:30', emoji: '🥾', label: '5-호수길 단축 (Blauherd→Stellisee→Grindjisee→Leisee→Sunnegga)', status: 'pending', detail: '⭐ Stellisee 마테호른 반영 명소 #2 · ⭐ Leisee 가족 친화 (잔디·벤치) · 6km, 2-2.5h, 누적 하강 ~300m, 평탄~완만' },
      { time: '16:30', emoji: '🚠', label: 'Sunnegga 푸니쿨라 하산 → Zermatt', status: 'pending' },
      { time: '17:00-19:00', emoji: '🏨', label: '호텔 휴식 — 샤워, 낮잠, 빨래 정리', status: 'pending', detail: '내일 융프라우 06:30 기상 대비 컨디션 ↑' },
      ...
      { time: '21:30', emoji: '🛌', label: '일찍 취침', status: 'pending', detail: '내일 06:30 기상 → 융프라우 08:04 Interlaken Ost 출발' },
    ],
  },
  {
  ...
  {
    day: 18, date: '6/29 (월)', phase: 'swiss', title: '⛰️ 융프라우요호 — Top of Europe (3,454m) ✅ 예매완료 #306398',
    icon: '⛰️', desc: '⭐ ✅ 예매완료 (jungfrau.ch #306398, 4명, STP 2nd class) · 08:04 Interlaken Ost → 08:43 Grindelwald Terminal → 09:13 Eigergletscher → 융프라우요호 (좌석예약 필수, 10분 전 도착) · 융프라우 체류 ~4h (09:45-13:45) — 유럽 最高 기차역, Aletsch 빙하 + Sphinx 전망대 · 13:45 융프라우 출발 → 14:20 Eigergletscher → 14:51 Interlaken Ost 복귀 · 오후 라우터브룬넨 폭포 골짜기 (72개 폭포, 톨킨 영감지) 옵션',
